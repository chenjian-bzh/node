...
if (ownsProcessState) {
  mainThreadSetup = require("internal/process/main_thread_only");
} else {
  workerThreadSetup = require("internal/process/worker_thread_only");
}
...
if (credentials.implementsPosixCredentials) {
  ...
  if (ownsProcessState) {
    ...
    process.initgroups = workerThreadSetup.unavailable("process.initgroups()");
...
if (!config.noBrowserGlobals) {
  ...
  const { TextEncoder, TextDecoder } = require("internal/encoding");
...
function setupPrepareStackTrace() {
  ...
  const {
    prepareStackTrace,
    fatalExceptionStackEnhancers: { beforeInspector, afterInspector }
